...
import { checkAuth, loadSheet, updateCell } from '../helpers/spreadsheet';
...
class App extends Component {
  ...
  handleAuth(authResult) {
    console.log("handleAuth " + authResult + "|" + authResult.error);
    if (authResult && !authResult.error) {
      this.setState({
        authenticated: true
      });
      loadSheet(this.onLoad.bind(this))
    } else {
      this.setState({
        authenticated: false
      })
  ...
  render() {
    return (
      <div className="app">
        <h1 className="brand">Rodney's Random Quotes”</h1>
  ...
  renderContent() {
    ...
    console.log("010 " + this.state.authenticated);
